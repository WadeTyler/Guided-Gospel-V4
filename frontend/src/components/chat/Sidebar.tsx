...
const Sidebar = ({currentSessionid, setCurrentSessionid}: {currentSessionid: string; setCurrentSessionid: React.Dispatch<React.SetStateAction<string>>;}) => {
  ...
  return (
    <>
      ...
      <button 
        className="lg:hidden fixed top-4 left-4 z-20 text-primary" 
      ...
      {/* Sidebar */}
      <div className={`bg-neutral-800 dark:bg-darkaccent ${sidebarOpen ? 'w-full' : 'hidden'} lg:w-48 h-full flex flex-col shadow-[rgba(0,0,0)_0px_2px_8px] gap-8 px-3 py-3 absolute z-10 pb-12 transition-all duration-300 ease-in-out lg:block`}>
        ...
        <h2 className="text-primary font-bold text-2xl text-center mt-1 lg:mb-8 ">Guided Gospel</h2>
  
        <section className="text-white flex flex-row gap-2 items-center text-lg cursor-pointer hover:scale-95 transition-all duration-300 ease-in-out lg:mb-8"
        onClick={() => {setCurrentSessionid(''); setSidebarOpen(false)}}
        >
          <RiChatNewFill />
          <p>New Chat</p>
        </section>
  
        <section className="flex flex-col gap-4 justify-start items-start overflow-y-auto h-[calc(100%-64px)] lg:h-[calc(100%-8rem)] hide-scrolbar">
          {sessionData && 
            sessionData.map((session) => (
              <div 
                className={`relative pb-4 border-b-[1px] border-b-primary last-of-type:border-none group cursor-pointer w-full ${currentSessionid === session.sessionid ? 'text-primary' : 'text-white'}`} 
                key={session.sessionid}
                onClick={() => {setCurrentSessionid(session.sessionid); setSidebarOpen(false);}}
              >
                <section className="hover:scale-95 transition-all duration-300 ease-in-out">
                ...
                {!deletingSession && 
                  <div className="absolute right-0 top-0 hover:text-red-500 text-xl" onClick={(e) => {
                    e.stopPropagation();
        ...
        {isDeletingAll && <div className="absolute top-0 left-0 w-screen h-screen bg-[rgba(0,0,0,.8)] z-50 flex items-center justify-center">
          <div className="bg-white dark:bg-darkbg rounded-2xl p-4 flex flex-col items-center justify-center gap-2 m-4">
            ...
            <section className="flex gap-4 items-center justify-center">
              ...
              <button className="neutral-btn" onClick={() => {
                setIsDeletingAll(false);
              }}>Cancel</button>
            </section>
          </div>
